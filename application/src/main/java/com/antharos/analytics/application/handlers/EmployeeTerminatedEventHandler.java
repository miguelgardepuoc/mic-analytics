...
public class EmployeeTerminatedEventHandler implements EmployeeEventHandler {
  ...
  public void handle(EmployeeEvent event) {
    log.info("Processing EMPLOYEE_TERMINATED event for employee {}", event.getId());
  ...
  public boolean canHandle(String eventName) {
    return EventNames.EMPLOYEE_TERMINATED.getDescription().equals(eventName);
  }
}
